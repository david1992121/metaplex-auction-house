...
export const assertValidBreakdown = (breakdown:any) => {
  const total:any = Object.values(breakdown).reduce(
    (sum:any, el:any) => (sum += el),
    0,
  );
...
export const generateRandomSet = (breakdown:any, dnp:any) => {
  let valid = true;
  let tmp:any = {};

  do {
    valid = true;
    const keys:any = shuffle(Object.keys(breakdown));
    keys.forEach((attr:any) => {
      const breakdownToUse = breakdown[attr];

      const formatted = Object.keys(breakdownToUse).reduce((f:any, key) => {
