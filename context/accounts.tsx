...
export const AccountsProvider: FC<{ children: ReactNode }> = ({ children }) => {
    ...
    ;(async () => {
      ...
      try {
        const [parsedAccounts, solAmount] = await Promise.all([
          connection.getParsedTokenAccountsByOwner(publicKey as any, { programId: TOKEN_PROGRAM_ID }),
        ...
        const accounts = parsedAccounts.value.reduce((acc: IAccounts, { account }) => {
          ...
          handleAccountChange(subscriptions, connection, publicKey as any, new PublicKey(mint))
          return acc
        }, {})
        ...
        alert("`Error fetching accounts`, icon: 'error'");
      }
  ...
  }, [connection, fetchAccounts, publicKey])
